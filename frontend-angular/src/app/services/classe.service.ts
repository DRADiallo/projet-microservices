...
// Nouvelle requête GraphQL pour la recherche de classes par libelle
const RECHERCHER_CLASSE_PAR_LIBELLE = gql`
  query($libelle: String!) {
    rechercheClasse(libelle: $libelle) {
      id
      libelle
      niveau
      specialite
...
export class ClasseService {
  ...
  // Nouvelle méthode pour rechercher des classes
  rechercherClasseParLibelle(libelle: string): Observable<Classe[]> {
    return this.classeClient
      .watchQuery<{ rechercheClasse: Classe[] }>({
        query: RECHERCHER_CLASSE_PAR_LIBELLE,
        variables: {
          libelle,
        },
      })
      .valueChanges.pipe(map((result) => result.data.rechercheClasse));
  }
